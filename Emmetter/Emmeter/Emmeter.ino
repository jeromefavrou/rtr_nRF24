...
#include <DI.hpp> //https://github.com/jeromefavrou/DI
...
void loop()
{
  ...
  if(bpSim.n_fixed() || magnetiqueSig.n_fixed())
  {
       radio->stopListening();
      bool stat = false;
      radio->write(&stat , sizeof(bool));
  
      if(!radio->write((char*)&stat , sizeof(bool)))
  ...
  if(bpSim.p_fixed() || magnetiqueSig.p_fixed() )
  {
      radio->stopListening();
      bool stat = true;
  ...
  delay(50);
    ...
    if(!radio->write((char*)false , sizeof(bool)))
          printLog<String>(F("fail send 0"));
    else
          printLog<String>(F("succes send 0"));
